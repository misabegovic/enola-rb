...
class FetcherTest < EnolaTest
  def test_fetches_verifies_and_caches_under_channel_and_version
    ...
    assert_equal File.join(@cache, "upstream", Enola::VERSION, "enola"), path
    assert File.executable?(path)
    assert File.exist?(File.join(@cache, "upstream", Enola::VERSION, "LICENSE"))
  ...
  def test_refuses_a_tarball_that_does_not_match_the_published_checksum
    ...
    channel = Enola::Channel.new(name: "upstream", release_base: base, version: Enola::VERSION, asset_prefix: "enola")

    error = assert_raises(Enola::Unverified) { fetcher(channel).fetch }

    assert_match(/did not match the checksum upstream published for #{Enola::VERSION}/, error.message)
  ...
  def test_offline_with_an_empty_cache_is_a_refusal_that_names_cache_and_release
    ...
    assert_match(%r{not in the cache at #{Regexp.escape(File.join(@cache, 'upstream', Enola::VERSION))}}, error.message)
